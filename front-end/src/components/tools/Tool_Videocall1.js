...
function Tool_Videocall(props){
    ...
    let audioSelect, videoSelect, startTime=null;
    ...
    useEffect(() => {
        ...
        getDevices().then( deviceInfos => gotDevices(deviceInfos) ).then(getStream()).catch( error => handleError(error) );

        audioSelect.onchange = getStream();
        videoSelect.onchange = getStream();

        getStream().then(getDevices()).then( deviceInfos => gotDevices(deviceInfos));
        ...
        socket.current.on('hey', (signal) => {
            trace(`SOMEONE CALLING`);
            callerSignal = signal;
            acceptCall();
        });
    ...
    function gotDevices(deviceInfos){
        ...
        for (const deviceInfo in deviceInfos) {
    ...
    function getStream(){
        ...
        return navigator.mediaDevices.getUserMedia(constraints).then( stream => gotStream(stream)).catch(error => handleError(error));
    }
    ...
    function startCall(){
        ...
        peer.on("signal", data => {
            socket.current.emit( "callUser", { roomID: roomID, signal: data });
            trace('Working......................');
        });

        peer.on('stream', stream => {
            trace('Partner Stream recieved to original user');
            if(w>992){
        ...
        socket.current.on("callAccepted", signal => {
            trace("Outgoing call got Accepted");
            peer.signal(signal);
        });
    }

    function acceptCall(){
        trace("Incoming Call Accepted");
        document.getElementById('startCall').style.display = "none";
        document.getElementById('endCall').style.display = "block";
        ...
        peer.on("signal", data => {
            trace('Working');
            console.log(data);
            socket.current.emit("acceptCall", { roomID: roomID, signal: data});
        });

        peer.on("stream", stream => {
            trace('Partner Stream recieved to candidate user');
            if(w>992){
                document.getElementById('localVideo').height = 150;
                document.getElementById('localVideo').width = 250;
            }
    ...
    function endCall(){
        ...
        socket.current.emit("disconnectCall", roomID)
            ...
            <div className="content-wrapper-fixed">
                <div className="" id="video-box">
                    <div className="users" id="localDiv">
                        <video id="localVideo" ref={localVideo} playsInline muted autoPlay></video>
                    </div>
                    <div className="users" id="candidateDiv">
                        <video width={w} height={h} id="candidateVideo" ref={candidateVideo} playsInline autoPlay></video>
                    </div>
                </div>
